import { CustomerSettings, EntityInstance, EntitySynonymType, MenuItem, Response } from "./types";
...
export function getMainValueFromEntitySynonym(input: EntitySynonymType[],value: string): string | null {
  ...
  export function getStandardObjects(settings: CustomerSettings): {[key: string]: any[]} {
    if (settings.nlu === "es") {
      return {
          MenuItems: buildMenuList(settings.Menu),
          MenuItemSizes: buildMenuItemSizes(settings.Menu),
          AlchoholTypes,
          SysNumber: SysNumber,
          Polite
          // Affirm,
      ...
      return {
        MenuItems: buildMenuListRasa(settings.Menu),
        MenuItemSizes: buildMenuItemSizes(settings.Menu),
  ...
  export function getRasaDomainLookup(settings: CustomerSettings): {[key: string]: any[]}  {
    return {
      MenuItems: buildMenuListRasa(settings.Menu),
      MenuItemSizes: generateLookupTable(buildMenuItemSizes(settings.Menu)),
  ...
  export function buildMenuList(Menu: MenuItem[]): EntityInstance[] {
  ...
  export function buildMenuListRasa(menu: MenuItem[]): string[] {
    return menu.map((m) => m.display_name )
  }
  export function buildMenuItemSizes(Menu: MenuItem[]): EntityInstance[] {
    const ret: EntityInstance[] = []
  ...
  export function buildMenuItemSizesRasa(Menu: MenuItem[]): string[] {
    const ret: string[] = []
    const tmpSizeArray: string[] = [];
    for (const menuItem of Menu) {
        for (const size of menuItem.sizes) {
