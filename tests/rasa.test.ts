import fs from 'fs';
import { CustomerSettings, Intent, RasaRule } from "../common/types"
import { Customer } from "../client/Customer"
import YAML from 'js-yaml';
import shell from 'shelljs';
import _ from "lodash";
import { toLowerCaseObj, extractQueryItems, getEntityMatches, getStateMods, isSubset, isValidIntent, getAllResponsesFromIntent, shouldResetContexts, isPrimitiveIntent, isNoContextSwitchIntent } from '../common/Utils';
import { RasaOptions, rasaParseMessage, rasaPredict, RasaPredictInput, RasaResponse } from '../common/rasa';
import { getValueFromEntityFill, isValidFillCase } from '../common/EntityFillUtils';
import { getStandardObjects, getCombinations, getStandardObjectValues, getRasaDomainLookup } from '../common/BaseDataTransformUtils';
...
describe("rasa converter", () => {
  Customer.Menu = toLowerCaseObj(Customer.Menu)
  describe("generatePermutations", () => {
    it("should generate all permutations of an empty array", () => {
      const items: any[] = [];
      const permutations = generatePermutations(items);
      expect(permutations).toEqual([[]]);
    });

    it("should generate all permutations of a single-item array", () => {
      const items = ["A"];
      const permutations = generatePermutations(items);
      expect(permutations).toEqual([[], ["A"]]);
    });

    it("should generate all permutations of a two-item array", () => {
      const items = ["A", "B"];
      const permutations = generatePermutations(items);
      expect(permutations).toEqual([[], ["B"], ["A"], ["A", "B"]]);
    });

    it("should generate all permutations of a three-item array", () => {
      const items = ["A", "B", "C"];
      const permutations = generatePermutations(items);
      expect(permutations).toEqual([
        [],
  ...
  describe("rasa endpoints", () => {
    it('Rasa healthcheck', async () => {
      ...
      expect(response).toBe("Hello from Rasa: 3.5.6")

    });
    it('Rasa model/parse and predict endpoints', async () => {
    ...
    it("rasa model/predict endpoint", async () => {
      const predictInput: RasaPredictInput[] = [
        {
          "event": "user",
          "timestamp": null,
          "metadata": {
      ...
      const predictRes = await fetch('http://0.0.0.0:5005/model/predict', {
        method: 'POST',
      ...
      expect(omitDeep(predictRes, ["timestamp", "latest_event_time"])).toMatchSnapshot()
    })
  ...
  describe("rasa setup", () => {
    ...
    it("can convert to nlu.yml", async () => {
      const yamlIntent = YAML.dump(
        {
          intent: jsonIntent.intent,
          examples: jsonIntent.examples.map(e => `- ${e}`).join('\n'),
        },
        {
          lineWidth: -1,
          noRefs: true,
        },
      ).replace("|-", "|"); // super dumb, but i dont know how to do it otherwise
      expect(yamlIntent).toEqual(`intent: greet\nexamples: |\n  - hey\n  - hello\n  - hi\n  - hello there\n  - good morning\n  - good evening\n  - moin\n  - hey there\n  - let's go\n  - hey dude\n  - goodmorning\n  - goodevening\n  - good afternoon\n`);

    })

    it("getRasaNluYaml", async () => {
      const testIntent: Intent = {
        id: "AlchoholPolicy",
        noFills: ["Alchohol@AlchoholTypes"],
        utterances: [
          "Do you serve ${Alchohol}",
          "Is ${Alchohol} allowed?",
        ],
      }
      const testIntent2: Intent = {
        id: "getCorkage",
        utterances: [
      ...
      const yamlIntent = intentsToNluYaml([testIntent, testIntent2], Customer);
      expect(yamlIntent).toMatchSnapshot()
    })
    it("getRasaDomainYaml", async () => {
      const testIntent: Intent = {
        id: "AlchoholPolicy",
        utterances: [
          "Do you serve ${Alchohol}",
      ...
      const yamlDomain = intentsToNluDomain(Customer, [testIntent, testIntent2]);
      expect(yamlDomain).toMatchSnapshot()
    })
    describe('intentsToStories', () => {
      it('should return an empty string for an empty array', () => {
        const result = intentsToStories(Customer, []);
        expect(result).toEqual('stories: []\n');
      });

      it('should throw an error for an invalid intent', () => {
        ...
        expect(() => intentsToStories(Customer, intents)).toThrow('invalid intent');
      });

      it('should return a valid Rasa stories YML string for valid intents', () => {
        const intents: Intent[] = [
          {
            id: 'greet',
            if: {
              booleanCondition: {
                ...
                true: {
                  ...
                  responses: [
                    {
                      text: "Welcome to our store. Can I help you with anything?"
                    }
                  ]
                },
                false: {
                  ...
                  responses: [
                    {
                      text: "Welcome back to our store. What can I help you with?"
                    }
                  ]
                }
              }
          ...
          },
          {
            ...
            if: {
              booleanCondition: {
                condition: {
                  "==": [
                    { var: "Store.Customer.hasItemsInCart" },
                    true
                  ]
                },
                true: {
                  ...
                  responses: [
                    {
                      text: "Thanks for shopping with us. Is there anything else I can help you with?"
                    }
                  ]
                },
                false: {
        ...
        const result = intentsToStories(Customer, intents);
        expect(result).toMatchSnapshot()
      });
    });

    describe('RasaParseMessage', () => {

      it('should parse a message and return a valid response', async () => {
        const message = 'Hello';
        const response: RasaResponse = await rasaParseMessage(message, Customer.Agent.rasaOptions);
    ...
    describe('RasaPredict', () => {
      ...
      it('should predict a message and return a valid response', async () => {
        const response: RasaResponse = await rasaPredict([predictInput], Customer.Agent.rasaOptions);
